...
{
	model = new Model("Data/Model/enemy2/robot.mdl");
...
//目標地点へ移動
void EnemySlime::MoveTarget(float elapsedTime, float moveSpeedRate, float turnSpeedRate)
{
	//ターゲット方向への進行ベクトルを算出
...
void EnemySlime::SetChaseState()
{
	state = State::Chase;
	stateTimer = MathUtils::RandomRange(3.0f, 5.0f);
}

void EnemySlime::UpdateChaseState(float elapsedTime)
{
	// 目標地点をプレイヤー位置に設定
