...
static MIDIState keyboard[2];
static MIDIState* keyboard_prev;
static MIDIState* keyboard_curr;
...
void devmidiUpdate() {
	...
	if (midiGetState(&keyboard[write_idx], 2)) {}
	...
	keyboard_curr = &keyboard[write_idx];
	keyboard_prev = &keyboard[1 - write_idx];
...
static bool SliderFloat(const char* id, MIDIState* curr, MIDIState* prev, int knob, float* v, float v_min, float v_max, const char* format, float power) {
	...
	linear_v = fminf(v_max, fmaxf(v_min, linear_v + del * knob_scale));
...
static bool ColorEditN(int components, const char* id, MIDIState* curr, MIDIState* prev, const int* knobs, float* v, float* v_defaults, ImGuiColorEditFlags flags) {
	...
	for (int i = 0; i < components; ++i) {
		...
		if ((convert_to_hsv || (flags & ImGuiColorEditFlags_InputHSV)) && i == 0)
			...
			edit[i] = fmaxf(0.0f, fminf(1.0f, edit[i]));
		if (v_defaults) {
			if (Release(curr, prev, knobs[i])) {
				edit[i] = v_defaults[i];
				touched = true;
...
void drawMidiKeyboard(const char* id, int note_min, int note_max) {
	ImGui::PushID(id);
	...
	for (int n = note_min; n <= note_max; ++n)
		if (!isBlackKey(n)) white_count++;

	float white_w = 20.0f, white_h = 100.0f;
	float black_w = white_w * 0.6f, black_h = white_h * 0.6f;

	bool mouse_down = ImGui::IsMouseDown(ImGuiMouseButton_Left);

	// --- 1. 先に「黒鍵優先」でホバー中のノートを1つだけ決める ---
	int hovered_note = -1; // 何もヒットしていなければ-1

	// 黒鍵を先にチェック（描画上、手前にあるので優先）
	{
		int white_idx = 0;
		for (int n = note_min; n <= note_max; ++n) {
			if (isBlackKey(n)) {
				ImVec2 p0 = { origin.x, origin.y + white_idx * white_w - black_w * 0.5f };
				ImVec2 p1 = { origin.x + black_h, origin.y + white_idx * white_w + black_w * 0.5f };
				if (ImGui::IsMouseHoveringRect(p0, p1)) {
					hovered_note = n;
					break; // 黒鍵が見つかったらそこで確定、白鍵は見なくていい
				}
			}
			else {
				white_idx++;
	...
	// 黒鍵に当たっていなければ、白鍵をチェック
	if (hovered_note == -1) {
		int white_idx = 0;
		for (int n = note_min; n <= note_max; ++n) {
			if (isBlackKey(n)) continue;
			ImVec2 p0 = { origin.x, origin.y + white_idx * white_w };
			ImVec2 p1 = { origin.x + white_h, origin.y + white_idx * white_w + white_w - 1 };
			if (ImGui::IsMouseHoveringRect(p0, p1)) {
				hovered_note = n;
				break;
			}
	...
	for (int n = note_min; n <= note_max; ++n) {
		...
		bool down = Down(keyboard_curr, keyboard_prev, n) || (n == hovered_note && mouse_down);

		draw->AddRectFilled(p0, p1, down ? IM_COL32(255, 180, 80, 255) : IM_COL32(255, 255, 255, 255));
	...
	for (int n = note_min; n <= note_max; ++n) {
		if (isBlackKey(n)) {
			...
			draw->AddRectFilled(p0, p1, down ? IM_COL32(255, 120, 0, 255) : IM_COL32(20, 20, 20, 255));
		}
		else {
			white_idx++;
		}
	...
	ImGui::Dummy({ white_count * white_w, white_h });
	ImGui::PopID();
}