...
ShapeRenderer::ShapeRenderer(ID3D11Device* device)
{
	...
	bufferDesc.ByteWidth = sizeof(CbMesh);
...
void ShapeRenderer::DrawCylinder(
    const DirectX::XMMATRIX& worldMatrix, // ワールド行列を受け取る
    float radius,
    float height,
    const DirectX::XMFLOAT4& color)
{
	...
	const DirectX::XMFLOAT4& color)
{
	...
	DirectX::XMVECTOR Direction = DirectX::XMVector3Normalize(Delta);
	// 回転行列
	...
	// スケーリング・平行移動行列
	DirectX::XMMATRIX S = DirectX::XMMatrixScaling(0.02f, 0.02f, length * 0.5f);
	DirectX::XMVECTOR MidPoint = DirectX::XMVectorScale(DirectX::XMVectorAdd(Start, End), 0.5f);
	DirectX::XMMATRIX T = DirectX::XMMatrixTranslationFromVector(MidPoint);
	DirectX::XMStoreFloat4x4(&instance.worldTransform, S * R * T);
}
