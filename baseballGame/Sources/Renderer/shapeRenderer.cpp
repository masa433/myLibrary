...
ShapeRenderer::ShapeRenderer(ID3D11Device* device)
{
	...
	//円錐メッシュ生成
	CreateWiredCylinderMesh(device, 1.0f, 0.0f, 0.0f, 1.0f, 16, wiredConeMesh);
	CreateSolidCylinderMesh(device, 1.0f, 0.0f, 0.0f, 1.0f, 16, true, solidConeMesh);
}

//ポイントライト描画
void ShapeRenderer::DrawPointLight(
...
//スポットライト描画
void ShapeRenderer::DrawSpotLight(
	const DirectX::XMFLOAT3& position,
	...
	const DirectX::XMFLOAT4& color)
{
	...
	//光の向きを可視化する線
	{
		...
		instance.wiredMesh = nullptr;
		instance.solidMesh = nullptr;
		//線はprimitive rendererで描画するため、メッシュは設定しない
	}

	//外側円錐
	{
		Instance& instance = instances.emplace_back();
		instance.wiredMesh = &wiredConeMesh;
		instance.solidMesh = nullptr; // ワイヤーのみ
		instance.color = color;

		float coneRadius = range * std::tan(outerAngle);

		//デフォルトのシリンダーはy軸方向なので、dirに向けて回転
		XMVECTOR up = XMVectorSet(0, 1, 0, 0);
		XMVECTOR rotAxis = XMVector3Cross(up, dir);
		float rotAngle = std::acos(XMVectorGetX(XMVector3Dot(up, dir)));

		XMMATRIX R;
		if(XMVector3LengthSq(rotAxis).m128_f32[0] < 1e-6f)
		{
			// dirがY軸と平行な場合
			R = XMMatrixRotationX(rotAngle > 1.0f ? XM_PI : 0.0f);
		}
		else
		{
			rotAxis = XMVector3Normalize(rotAxis);
			R = XMMatrixRotationAxis(rotAxis, rotAngle);
		}

		XMMATRIX S = XMMatrixScaling(coneRadius, range, coneRadius);
	...
	// ===== 3. 内側円錐 =====
	{
		...
		instance.wiredMesh = &wiredConeMesh;
		instance.solidMesh = nullptr;
		instance.color = { color.x, color.y, color.z, color.w * 0.5f }; // 少し薄く
		...
		XMMATRIX S = XMMatrixScaling(innerRadius, range, innerRadius);
...
void ShapeRenderer::CreateWiredCylinderMesh(ID3D11Device* device, float radius1, float radius2, float start, float height, int subdivisions, WiredMesh& outMesh)
{
	std::vector<DirectX::XMFLOAT3> vertices;
	...
	// メッシュ生成
	CreateWiredMesh(device, vertices, outMesh);
}
...
void ShapeRenderer::CreateSolidCylinderMesh(ID3D11Device* device, float radius1, float radius2, float start, float height, int subdivisions, bool cap, SolidMesh& outMesh)
{
	int stacks = 1;
	int slices = subdivisions;
	float stack_height = height / stacks;
	...
	CreateSolidMesh(device, vertices, indices, outMesh);
}
	...
	const DirectX::XMFLOAT3& lightDirection)
{
	...
	// ソリッド
	{
		...
		{
			if (!instance.solidMesh) continue; // nullチェックを追加
	...
	// ワイヤーフレーム
	{
		...
		for (const Instance& instance : instances)
		{
			if (!instance.wiredMesh) continue; // nullチェックを追加
